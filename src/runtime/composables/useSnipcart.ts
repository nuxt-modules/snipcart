...
export const useSnipcart = () => {
   ...
   */
  const bindProductItemCustom = (bindProductItemCustom: []) => {
    const fields: Map = {}

    bindProductItemCustom.forEach((field: Record<string, string>, index: number) => {
  ...
  const setLanguage = async (lang: string, _translations = {}) => {
    ...
    const translations = nuxtApp.$config.public.snipcart.translations as Record<string, unknown> || {}
    const lang_translations = translations[lang] as Record<string, unknown> || {}
